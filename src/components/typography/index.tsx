...
export function H2({ children, className }: PropsWithChildrenAndClassName) {
  return (
    <h2
      className={cn(
        "mt-10 scroll-m-20 border-b border-b-zinc-200 pb-2 text-2xl md:text-3xl font-bold tracking-tight transition-colors first:mt-0 dark:border-b-zinc-700",
        className
      )}
    >
      {children}
...
export function Large({ children, className }: PropsWithChildrenAndClassName) {
  return (
    <div className={cn("md:text-lg font-medium text-zinc-900", className)}>
      {children}
    </div>
  );
}
...
export function Code({ children, className }: PropsWithChildrenAndClassName) {
  return (
    <code
      className={cn(
        "relative rounded bg-rose-100 px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold text-zinc-900",
        className
      )}
    >
      {children}
